...
class SEBlock(nn.Module):
    def __init__(self,channel,reduction=16):
        super().__init__()
        self.squeeze=nn.AdaptiveAvgPool3d(1)#空間サイズを1にする
    ...
    def forward(self,x):
        b,c=x.shape[:2]
        y=self.squeeze(x).view(b,c)#squeeze+[b,cに変換]
        y=self.excite(y).view(b,c,1,1,1)#excite+[b,c,1,1,1に変換]
        return y
...
class SEResNetBlock(nn.Module):
    def __init__(self,
        ...
        self.se=SEBlock(dim_list[3],reduction=reduction)
        self.res_conv = nn.Conv3d(dim_list[0], dim_list[3], 1).apply(weights_init_normal) \
            if dim_list[0] != dim_list[3] else nn.Identity()
    ...
    def forward(self, x, t):
        ...
        h = self.block3(h)
        # print(h.shape)
        channel_weight=self.se(h)
        h=h*channel_weight.expand_as(h)
        return h + self.res_conv(x)
